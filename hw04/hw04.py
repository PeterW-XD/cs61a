def shuffle(s):
    ...
    first_half = s[:mid]
    second_half = s[mid:]
    return [x for a, b in zip(first_half, second_half) for x in (a, b)]
...
def balanced(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    # Base case: planet is self-balanced
    if is_planet(m):
        return True
    # Recursive case: not planet? check if balanced and call balanced() with branches
    else:
        left_mass = total_mass(end(left(m)))
        right_mass = total_mass(end(right(m)))
        left_torque = left_mass * length(left(m))
        right_torque = right_mass * length(right(m))
