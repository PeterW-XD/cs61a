...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    tru_para = [para for para in paragraphs if select(para)]
    return tru_para[k] if k < len(tru_para) else ""
...
def about(keywords):
    ...
    def is_contain_keywords(paragraph):
        split_paragraph = split(lower(remove_punctuation(paragraph)))
        for word in keywords:
            if word in split_paragraph:
                return True
        return False
    return is_contain_keywords
...
def accuracy(typed, source):
    ...
    if typed_words == [] and source_words == []:
        ...
        matched = 0
        for typed_w, source_w in zip(typed_words, source_words):
            if typed_w == source_w:
                matched += 1
        res = 100 * matched / len(typed_words)
...
def minimum_mewtations(typed, source, limit):
    ...
    if typed[0] == source[0]: # Feel free to remove or add additional cases
        ...
        min2 = 1 + minimum_mewtations(remove, source, limit-1) # remove character should be checked again
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    times = []
    for timestamps_p in tpp:
        times_p = []
        for i in range(len(timestamps_p)-1):
            times_p.append(timestamps_p[i+1] - timestamps_p[i]) 
...
def fastest_words(words_and_times):
    ...
    player_indices = len(times)  # contains an *index* for each player
    word_indices = len(words)    # contains an *index* for each word
    # BEGIN PROBLEM 10
    "*** YOUR CODE HERE ***"
    res = [[] for _ in range(player_indices)]
    for j in range(word_indices):
        smallest = times[0][j]
        smallest_idx = 0
        for i in range(1, player_indices):
            if times[i][j] < smallest:
                smallest = times[i][j]
                smallest_idx = i
        res[smallest_idx].append(words[j])        
